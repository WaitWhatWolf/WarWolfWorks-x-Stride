...
namespace WarWolfWorksxS.NyuEntities.Statistics
{
    /// <summary>
    /// <see cref="INyuStat"/> which is removed after a given countdown.
    /// </summary>
    [Serializable]
    public sealed class CountdownStat : INyuStat, INyuTaskExecutable
    {
        /// <summary>
        ...
        void INyuStat.OnAdded(Stats to) { pv_Parent = to; }

        void INyuStat.OnRemoved(Stats to) { }

        Func<Task> INyuTaskExecutable.Task => async () =>
        {
            await System.Threading.Tasks.Task.Delay(TimeSpan.FromSeconds(Countdown));
            pv_Parent.RemoveStat(this);
        };

        bool INyuTaskExecutable.TaskRunning { get; set; }
        AsyncScript INyuTaskExecutable.Script { get; set; }

        private Stats pv_Parent;
